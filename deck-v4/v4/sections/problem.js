...
registerSection({
  ...
  mobilePinLength: 1.35,
  scrubMobile: true,
  build(gsap, ctx, { mobile }) {
    const section = document.getElementById("problem");
    const anchor = document.getElementById("problem-seat-anchor");
    ...
    if (ctx.product && anchor) {
      tl.to(ctx.product, {
        x: () => ctx.rectVars(anchor, { rotate: mobile ? -7 : -11, scale: mobile ? 0.62 : 0.66 }).x,
        y: () => ctx.rectVars(anchor, { rotate: mobile ? -7 : -11, scale: mobile ? 0.62 : 0.66 }).y,
        xPercent: -50,
        yPercent: -50,
